...
\begin{align*}
    d = ||\vec{pc_{xy}} - \vec{p_{xy}} || * 0.7 + ||\vec{pc_z} -
    \vec{p_z} || * 0.3\\
\end{align*}
...
\begin{figure}
    \begin{tikzpicture}
        \coordinate [label=left:$\vec{c_p}$] (A) at (-2.5,0.2);
        \coordinate [label=right:$\vec{f_d}$] (B) at (5, 0 );
        \coordinate [label=right:$d$] (F) at (-7, 1.25);
    ...
    this line. to find its accuracy the distance $d$ should be solved. Note that
...
contact point and the destinatiaon point. See Figure \ref{fig:accuracy} for a
visualisation of the problem. This idea makes for a basic linear algebra
problem that needs to be resolved.
\begin{align*}
    d = \frac{||(\vec{p_{xy}} - cp_{xy}) \times (\vec{p_{xy}} -
    \vec{f_d})||}{\|\vec{fd_{xy}}- \vec{cp_{xy}}\|}
\end{align*}
...
decision towards what a good retractionpoint is. It is found by the following
equation.

\begin{align*}
    \arg\max_\vec{p} = 1 - \delta * f() + \delta * g()
\end{align*}
