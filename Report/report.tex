...
experiments have been conducted on a RoboCup Nao H21, version $3.3$ and $4.0$.
...
transformations to walk through each component recursively while calculating the
center of mass of the entire robot.
...
\begin{itemize}
    \item $Front_t = (1-s) * Front_{t-1} + s * (\textit{LFsrFL} + \textit{LFsrFR})$
    \item $\textit{Left}_t = (1-s) * \textit{Left}_{t-1} + s *(\textit{LFsrFL} + \textit{LFsrRL})$
    \item $\textit{Right}_t = (1-s) * \textit{Right}_{t-1} + s *(\textit{LFsrFR} + \textit{LFsrRR})$
    \item $\textit{Back}_t = (1-s) * \textit{Back}_{t-1} + s *(\textit{LFsrRL} + \textit{LFsrRR})$ 
\end{itemize}
...
\begin{itemize}
    \item $e(t)_x = (\textit{Front}_t - \textit{Back}_t)$
    \item $e(t)_y = (\textit{Right}_t - \textit{Left}_t)$
    \item $P_{out} x = e(t)_xK_p$
    \item $P_{out} y = e(t)_yK_p$
    \item $D_{out} x = \frac{e(t)_x - e(t-1)_x}{\textit{timeTaken}}K_d$
    \item $D_{out} y = \frac{e(t)_y - e(t-1)_y}{]textit{timeTaken}}K_d$
    \item $\textit{Offset}$ $\textit{Hip}$ $\textit{Pitch}$ $= P_{out}x + D_{out}x$ 
    \item $\textit{Offset}$ $\textit{Hip}$ $R\textit{oll}$ $=  P_{out}y + D_{out}y$ 
\end{itemize}
...
\begin{itemize}
	\item $\textit{Offset Hip Pitch} = (P_{out}x + D_{out}x) * \textit{timeTaken}$ 
	\item $\textit{Offset Hip Roll} =  (P_{out}y + D_{out}y) * \textit{timeTaken}$ 
\end{itemize}
...
Pseudocode for the entire algorithm can be seen in algorithm \ref{code:ik}.
...
\begin{figure}[htbp]
  \centering
  \makebox[\textwidth] {
    \includegraphics[width=1.3\textwidth]{pics/ik_2.pdf}
  }
...
\begin{figure}[htbp]
  ...
  \caption{Visualistation of relevance of all tried retraction points. The red
      ...
      this case the ball should be kicked straight forward.
         }
...
$\begin{bmatrix} 1 &  0 & 0 \end{bmatrix}^T$ and a center of mass positioning of
...
\frac{1}{\sqrt{2}} & 0 \end{bmatrix}^T$. Using the same $\delta$ value as before gave us
...
case the ball should be kicked straight ahead again ( direction = $\begin{bmatrix} 1 &  0 & 0 \end{bmatrix}^T$). All blue dots are evaluated retraction points and the red stars are the points that are the most
...
each other, equation \ref{eq:distance} only considers the value of $x$ when the ball should go
straight ahead.
...
Now that all subproblems of making a dynamic kick seem to be solved succesfully, there are still some future improvements we would like to work on.
