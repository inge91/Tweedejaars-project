...
$\begin{bmatrix} 1 \\  0 \\ 0 \end{bmatrix}$ and a center of mass positioning of
    ...
    chosen retraction point the Nao executes. For this experiment we chose a
    $\delta = 0.001$, so that distance was more important than accuracy.In this
    case this gave the right effect; both giving a accurate and powerful
    kicking speed.

Another experiment shows the same idea but, instead of kicking straight ahead,
the direction was 45 degrees to the right, or $\begin{bmatrix} \sqrt{2}  \\
\sqrt{2} \\ 0 \end{bmatrix}$. Using the same $\lambda$ value as before gave us
the result as can be seen in \ref{fig: retraction_plot2}
