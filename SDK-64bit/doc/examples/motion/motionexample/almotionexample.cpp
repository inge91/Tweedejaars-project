    ...
    const std::string& name): AL::ALModule(broker, name)
{
  ...
  functionName("walkTo", "ALMotionExample", "Walks using the walkTo command and shows odometry.");
  BIND_METHOD(ALMotionExample::walkTo);
...
void ALMotionExample::cartesianArm1() {
  ...
  // SPACES: SPACE_TORSO 0, SPACE_WORLD 1, SPACE_NAO 2
...
void ALMotionExample::poseInit() {
  ...
  {
    targetAngles[i] = (float)targetAngles[i] * AL::Math::TO_RAD;
  }
...
void ALMotionExample::poseZero() {
  ...
  int numBodies = proxy->getJointNames(names).size();

  // We prepare a collection of floats
  std::vector<float> targetAngles;
  targetAngles.assign(numBodies, 0.0f);  // fill with zeros
...
void ALMotionExample::walk() {
  ...
  // or disable
  // proxy->setWalkArmsEnable(false, false)
  ...
  // walkking.
  //
  // First, disactivate the automatic left arm motion
  proxy->setWalkArmsEnable(false, true);
...
void ALMotionExample::walkToCustomization() {
  ...
  proxy->walkTo(x, y, theta, AL::ALValue::array(
