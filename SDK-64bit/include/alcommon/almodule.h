...
namespace AL
{
  ...
  public:
     ...
     * \param pID method's ID to stop
     */
    virtual void stop(const int &pID);

    /**
     * \brief Check if the user call stop.
     * \param pID method's ID to check
     * \return true if used call stop, false otherwise
     */
    bool isStopRequired(const int &id = -1);

    /**
     * \brief Wait until the end of a long running method using the ID that was
           returned from a method started with 'post'
     * \param pID the id of the methods
     * \param pTimeout wait timeout. If 0 wait indefinately.
     ...
     * \param pID the id of the methods that was returned by 'post'
