...
def get_angles(foot_to_torso, leg):
    ...
    delta_footroll = atan2(foot_to_hip_orth[1, 3], foot_to_hip_orth[2, 3])
    ...
    delta_hipyaw = atan2(-rot_component[0, 1], rot_component[1, 1])
    delta_hippitch = atan2(-rot_component[2, 0], rot_component[2, 2])
    delta_hiproll = delta_x - (pi / 4.0)
...
def set_position(mp, leg, location, speed):
    ...
    angles = get_angles(location_matrix)
    angles = angles.items()
